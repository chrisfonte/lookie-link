...
3. **Principal:** Operator direction (2026-07-15): an instance COULD be multi-user, so the contract is written against the roadmap identity taxonomy (operator/user/agent/share-recipient) with real per-principal permissions from day one — v1 runtime ships single-operator + agent tokens as one principal provider, never a special schema. Receipts must become verifiable (signed/MAC) before a second real user principal is enabled. Ratify in #92.
