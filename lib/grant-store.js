...
const { expandHome, toPosixPath } = require('./path-utils');
...
function serializeGrant(grant, now = Date.now()) {
  const { tokenHash: _tokenHash, ...projection } = grant;
  return {
    ...projection,
...
function buildIssueLink(issueId) {
  ...
  const projectKey = normalized.includes('-') ? normalized.split('-', 1)[0] : null;
  if (!projectKey) {
    return normalized;
  }

  return `[${normalized}](/${projectKey}/issues/${normalized})`;
}

function describeGrantScope(grant) {
  if (!Array.isArray(grant.paths) || grant.paths.length === 0) {
    return 'whole repo';
  }

  if (grant.paths.length === 1) {
    return `path \`${grant.paths[0]}\``;
  }

  return `${grant.paths.length} paths`;
}
...
function buildIssueComment(action, grant, extra = {}) {
  ...
  const permissionLabel = [
    ...
    (grant.permissions.write || grant.permissions.edit) && 'write',
  ...
  if (action === 'created') {
    lines.push(`Issued Lookie-Link grant \`${grant.id}\` for ${issueLink}.`);
  } else if (action === 'renewed') {
    lines.push(`Renewed Lookie-Link grant \`${grant.id}\` for ${issueLink}.`);
  } else if (action === 'revoked') {
    lines.push(`Revoked Lookie-Link grant \`${grant.id}\` for ${issueLink}.`);
  } else if (action === 'expired') {
    lines.push(`Expired Lookie-Link grant \`${grant.id}\` for ${issueLink}.`);
  }

  lines.push('');
  lines.push(`- Repo: \`${grant.repoId}\``);
  lines.push(`- Scope: ${scopeLabel}`);
  lines.push(`- Permissions: ${permissionLabel}`);
  lines.push(`- Target company: \`${grant.targetCompanyId}\``);
  lines.push(`- Expires: ${grant.expiresAt}`);

  if (grant.approvalId) {
    lines.push(`- Approval: \`${grant.approvalId}\``);
  }

  if (action === 'revoked') {
    lines.push(`- Revocation reason: ${grant.revocationReason}`);
  } else {
  ...
  return {
    issueId: grant.sourceIssueId,
    markdown: `${lines.join('\n')}\n`,
  };
}

function recordExpiredGrantAuditEvents(store, now = Date.now()) {
  let changed = false;

  for (const grant of store.grants) {
    if (computeGrantState(grant, now) !== 'expired') {
      continue;
    }

    const existing = store.auditEvents.some((event) => event.grantId === grant.id && event.type === 'grant.expired');
    if (existing) {
      continue;
    }

    store.auditEvents.push(createAuditEvent(grant.id, 'grant.expired', null, {
      expiresAt: grant.expiresAt,
      issueComment: buildIssueComment('expired', grant, {
        note: 'Grant expired on schedule.',
      }),
    }));
    changed = true;
  }
...
class GrantStore {
  ...
  static fromAccessConfig(rawGrantsConfig) {
    ...
    const storePath = typeof grantsConfig.storePath === 'string' && grantsConfig.storePath.trim()
      ? path.resolve(expandHome(grantsConfig.storePath.trim()))
      : null;
    const projectionPath = typeof grantsConfig.projectionPath === 'string' && grantsConfig.projectionPath.trim()
      ? path.resolve(expandHome(grantsConfig.projectionPath.trim()))
      : null;
    ...
    return new GrantStore({
      ...
      repoOwners,
      adminTokens,
      repoRoots: grantsConfig.repoRoots && typeof grantsConfig.repoRoots === 'object'
        ? Object.fromEntries(
            Object.entries(grantsConfig.repoRoots)
              .map(([repoId, repoRoot]) => [String(repoId || '').trim(), typeof repoRoot === 'string' && repoRoot.trim()
                ? path.resolve(expandHome(repoRoot.trim()))
                : null])
              .filter(([repoId, repoRoot]) => repoId && repoRoot)
          )
        : {},
    });
  }
  ...
  authenticateGrantToken(secret, source, queryToken) {
    ...
    if (recordExpiredGrantAuditEvents(store, now)) {
      changed = true;
    }
  ...
  listGrants(filters = {}) {
    const store = this.readStore();
    const now = Date.now();
    const changed = recordExpiredGrantAuditEvents(store, now);
