...
function splitReference(value) {
  ...
  // Already viewer-routed absolute paths (authored /asset/..., /view/..., etc.)
  // must pass through untouched — re-resolving them against the current repo
  // double-prefixes the route (/asset/<repo>/asset/<repo>/... → 404).
  if (/^\/(?:asset|view|raw|embed|api)\//.test(raw)) {
    return null;
  }
