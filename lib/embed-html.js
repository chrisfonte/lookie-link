...
const { decorateRenderedHtmlForAnnotations, detectRenderMode } = require('./renderer');
...
function addThemeInjection(document, options) {
  // Viewport render mode ships a different embed runtime: the frame scrolls itself,
  // so native position:sticky/fixed/:target/lightbox work and the content-height,
  // anchor-offset, image-overlay and :target bridges are all switched OFF. Every
  // divergence below is build-time gated on `viewport`; with it false the emitted
  // runtime is byte-for-byte today's content-height reader.
  const viewport = options.renderMode === 'viewport';
  ...
  // Default mode keeps EXACTLY the original element set (no filter, no extra
  // element) so the emitted <style> stays byte-for-byte identical to today. The
  // viewport rule is pushed only when viewport, never as an empty placeholder.
  const styleParts = [
  ...
  if (viewport) {
    // Viewport mode: the embedded document is the scroller, so contain overscroll
    // to this frame — a flick past the top/bottom edge does not chain out to the
    // viewer page behind it.
    styleParts.push('html.lookie-embedded-viewport, html.lookie-embedded-viewport body { overscroll-behavior: contain; }');
  }
  ...
  function viewportActiveId() {
    ...
    for (var v = 0; v < nodes.length; v += 1) {
      ...
      if (node.tagName === 'SECTION') {
        var inner = node.querySelector('h1,h2,h3,h4');
        if (!inner || inner.id) continue;
      }
      seen[node.id] = 1;
      if (node.getBoundingClientRect().top <= 140) activeId = node.id;
    }
    return activeId;
  }
  var vpActiveTick = false;
  var vpLastActive = '';
  function postViewportActive() {
    if (!framed) return;
    var id = viewportActiveId();
    if (id && id !== vpLastActive) {
      vpLastActive = id;
      window.parent.postMessage({ type: 'lookie-link:toc-active', id: id }, '*');
    }
  }
  window.addEventListener('scroll', function () {
    if (vpActiveTick) return;
    vpActiveTick = true;
    window.requestAnimationFrame(function () { vpActiveTick = false; postViewportActive(); });
  }, { passive: true });
  document.addEventListener('DOMContentLoaded', postViewportActive);
  window.addEventListener('load', postViewportActive);
` : ''}}());`;
...
function addAnnotations(document, options) {
  ...
  // Gate only: hide annotate buttons until the parent posts annotation-mode.
  // Do NOT inject the annotations bootstrap, initializer, or /public/annotations.js.
  // /embed runs in a sandboxed frame without allow-same-origin (opaque origin
  // "null"), so any fetch to /api/annotations/… is blocked by CORS and leaves
  // console noise that hangs Playwright networkidle. Annotation counts and the
  // client live on the outer /view page for non-embedded documents instead.
...
function transformEmbedHtml(source, options) {
  ...
  // Declared render mode drives which embed runtime ships. Detect from the raw
  // source (same tolerant check /view uses) so both sides agree on the mode.
  const renderMode = detectRenderMode(source);
  addThemeInjection(document, { ...options, renderMode });
