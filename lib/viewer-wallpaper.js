...
let _catalog = {};
let _aliases = {};
let _defaults = { panelOpacity: 72, blur: 12 };
let _themes = [];

function setWallpaperCatalog(catalog, themes = [], defaults = null) {
  _catalog = catalog && typeof catalog === 'object' ? catalog : {};
  if (defaults && typeof defaults === 'object') _defaults = { panelOpacity: defaults.panelOpacity, blur: defaults.blur };
  _themes = Array.isArray(themes) ? themes : [];
  _aliases = {};
...
function currentDefaults() {
  return { panelOpacity: _defaults.panelOpacity, blur: _defaults.blur };
}
...
function publicCatalog() {
  ...
  for (const [slug, sets] of Object.entries(_catalog)) {
    out[slug] = {
      ...
      panelOpacity: sets.panelOpacity === undefined ? _defaults.panelOpacity : sets.panelOpacity,
      blur: sets.blur === undefined ? _defaults.blur : sets.blur,
    };
  }
...
function viewerWallpaperRuntime() {
  ...
  // Glass defaults come from config (global block, overridable per theme).
  function defaults() {
    var sets = catalog[scheme()];
    return {opacity: sets && Number.isInteger(sets.panelOpacity) ? sets.panelOpacity : 72, blur: sets && Number.isInteger(sets.blur) ? sets.blur : 12};
  }
  var state = load();
  var catalogKey = '';
  ...
  function load() {
    ...
    // opacity/blur stay null until the reader touches a slider, so the
    // configured default (which may differ per theme) applies until then.
    var s = {choices: {}, off: false, opacity: null, blur: null};
  ...
  function effectiveOpacity() { return state.opacity === null ? defaults().opacity : state.opacity; }
  function effectiveBlur() { return state.blur === null ? defaults().blur : state.blur; }
  ...
  function apply() {
    ...
    root.style.setProperty('--wallpaper-panel', effectiveOpacity() + '%');
    root.style.setProperty('--wallpaper-blur', effectiveBlur() + 'px');
    // Menu.
    ...
    choice.value = id;
    opacity.value = effectiveOpacity();
    blur.value = effectiveBlur();
    menu.querySelector('[data-wallpaper-opacity-value]').textContent = effectiveOpacity() + '%';
    menu.querySelector('[data-wallpaper-blur-value]').textContent = effectiveBlur() ? effectiveBlur() + ' px' : 'Off';
  }
  function pick(id) {
    urlWallpaper = null;
    if (id === 'none') {
  ...
  menu.querySelector('[data-wallpaper-reset]').addEventListener('click', function () {
    ...
    state.blur = null;
    save();
    urlApi.set('lookie-panel', effectiveOpacity());
    urlApi.set('lookie-blur', effectiveBlur());
    apply();
  });
...
// Live reload. Watches the config file's directory (editors replace the file,
// so a watch on the file itself goes stale) and every configured wallpaper
// folder; on any change, after a short quiet period, rescans and swaps the
// catalog. Cost: one readdir per folder per change; nothing per request.
// Pages already open pick the new catalog up on their next load.
function watchWallpapers({ fs, path, reload, log = console.log, warn = console.warn, quietMs = 400 }) {
  let watchers = [];
  let timer = null;
  function close() {
  ...
  function run() {
    ...
    try {
      ...
      warn(`Warning: wallpaper reload failed (${error.message}); keeping the previous catalog`);
      return;
    }
    arm(result.watch);
    log(`Themes reloaded: ${result.summary}`);
  }
  const initial = reload();
  arm(initial.watch);
  log(`Themes: ${initial.summary} (live reload on)`);
