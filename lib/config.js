...
const CONFIG_FILENAME = 'lookout.yaml';
...
/**
 * Load lookout.yaml config file.
 * Search order: LOOKOUT_CONFIG env → ~/.config/lookout/lookout.yaml → project root (fallback)
...
function loadConfigFile() {
  ...
  if (process.env.LOOKOUT_CONFIG) {
    candidates.push(path.resolve(expandHome(process.env.LOOKOUT_CONFIG)));
  }

  // 2. User config dir (~/.config/lookout/lookout.yaml)
  candidates.push(path.join(os.homedir(), '.config', 'lookout', CONFIG_FILENAME));
...
const DEFAULT_REPOS = {
  // Configure your repos in lookout.yaml or ROOT_MAPPINGS env var.
...
function loadRootMappings() {
  // Priority: ROOT_MAPPINGS env > lookout.yaml > defaults
...
function getPort() {
  // Priority: PORT env > lookout.yaml > default
