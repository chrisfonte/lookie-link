...
class ManagedRepoStore {
  ...
  // Soft-deleted records, newest first. Each is { trashId, originalPath,
  // deletedAt, size }; malformed records are skipped rather than failing the
  // listing. (API roadmap R3, 2026-09-23.)
  async listTrash(repo) {
    let trashRoot;
    try { trashRoot = this.resolvePath(repo, repo.policy.trashDirName); } catch (error) {
      if (error && error.code === 'ENOENT') return [];
      throw error;
    }
    let names = [];
    try { names = await fs.promises.readdir(trashRoot.absolutePath); } catch (error) {
      if (error && error.code === 'ENOENT') return [];
      throw error;
    }
    const records = [];
    for (const name of names) {
      const recordDir = path.join(trashRoot.absolutePath, name);
      try {
        const metadata = JSON.parse(await fs.promises.readFile(path.join(recordDir, 'metadata.json'), 'utf8'));
        if (!metadata || metadata.trashId !== name || typeof metadata.originalPath !== 'string') continue;
        let size = null;
        try { size = (await fs.promises.stat(path.join(recordDir, 'payload'))).size; } catch (_) {}
        records.push({ trashId: name, originalPath: metadata.originalPath, deletedAt: metadata.deletedAt || null, size });
      } catch (_) { continue; }
    }
    records.sort((a, b) => String(b.deletedAt || '').localeCompare(String(a.deletedAt || '')) || a.trashId.localeCompare(b.trashId));
    return records;
  }
