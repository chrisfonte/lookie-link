...
class ApiKeyStore {
  ...
  recordAuditEvent(type, accessContext, target, metadata = {}) {
    ...
    const event = createAuditEvent(type, {
      ...
      resourceKind: target && target.repo ? 'repo'
        : target && typeof target.resourceKind === 'string' ? target.resourceKind : null,
      resourceId: target && target.repo ? String(target.repo)
        : target && typeof target.resourceId === 'string' ? target.resourceId : null,
      ...
      submissionId: typeof metadata.submissionId === 'string' ? metadata.submissionId : null,
      templateVersion: Number.isSafeInteger(metadata.templateVersion) ? metadata.templateVersion : null,
      schemaDigest: typeof metadata.schemaDigest === 'string' ? metadata.schemaDigest : null,
    });
