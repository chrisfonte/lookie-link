...
function buildAssetHref(repo, relativePath) {
  const repoPart = encodePathSegment(repo);
  const rel = toPosixPath(relativePath);
  if (!rel) {
    return `/asset/${repoPart}`;
  }

  return `/asset/${repoPart}/${rel.split('/').map(encodePathSegment).join('/')}`;
}

function buildEditHref(repo, relativePath) {
  const repoPart = encodePathSegment(repo);
  const rel = toPosixPath(relativePath);
  if (!rel) {
    return `/edit/${repoPart}`;
  }

  return `/edit/${repoPart}/${rel.split('/').map(encodePathSegment).join('/')}`;
}

function buildSaveHref(repo, relativePath) {
  const repoPart = encodePathSegment(repo);
  const rel = toPosixPath(relativePath);
  if (!rel) {
    return `/api/save/${repoPart}`;
  }

  return `/api/save/${repoPart}/${rel.split('/').map(encodePathSegment).join('/')}`;
}

function buildPreviewHref(repo, relativePath) {
  const repoPart = encodePathSegment(repo);
  const rel = toPosixPath(relativePath);
  if (!rel) {
    return `/api/preview/${repoPart}`;
  }
