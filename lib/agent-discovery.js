...
const ENDPOINT_TEMPLATES = Object.freeze({
  ...
  publishList: '/api/publish',
  publishGet: '/api/publish/:slug',
  ...
  kits: '/api/kits',
  kit: '/api/kits/:name',
  kitStylesheet: '/kit/:name/kit.css',
  kitFile: '/kit/:name/files/:file',
  forms: '/forms',
...
function buildDiscoveryState({
  ...
  const publishReadAllowed = publishEnabled
    && Boolean(publishedRepo)
    && canAccessRepo(accessContext, 'view', publishedRepo)
    && enabledRoute(routeAvailability, 'publishList');
  ...
  const capabilities = {
    ...
    publishRead: publishReadAllowed,
    ...
    // Hosted HTML kits: at least one kit loaded and the list/show routes exist.
    kits: Boolean(kitsEnabled
      && enabledRoute(routeAvailability, 'kits')
      && enabledRoute(routeAvailability, 'kit')),
  };
  ...
  if (capabilities.publishRead) {
    endpoints.publishList = ENDPOINT_TEMPLATES.publishList;
    if (enabledRoute(routeAvailability, 'publishGet')) endpoints.publishGet = ENDPOINT_TEMPLATES.publishGet;
  }
  ...
  if (capabilities.kits) {
    endpoints.kits = ENDPOINT_TEMPLATES.kits;
    if (enabledRoute(routeAvailability, 'kit')) endpoints.kit = ENDPOINT_TEMPLATES.kit;
    if (enabledRoute(routeAvailability, 'kitStylesheet')) endpoints.kitStylesheet = ENDPOINT_TEMPLATES.kitStylesheet;
    if (enabledRoute(routeAvailability, 'kitFile')) endpoints.kitFile = ENDPOINT_TEMPLATES.kitFile;
  }
