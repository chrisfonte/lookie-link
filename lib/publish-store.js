...
function isHtmlPublishPath(filePath) {
  const lower = String(filePath || '').toLowerCase();
  return lower.endsWith('.html') || lower.endsWith('.htm');
}

function linkHasStylesheetRel(tag) {
  return /\brel\s*=\s*(["']?)stylesheet\1(?:\s|>|\/)/i.test(tag)
    || /\brel\s*=\s*stylesheet(?:\s|>|\/)/i.test(tag);
}

function linkHasDataKit(tag) {
  return /\bdata-kit(?:\s*=|\s|\/|>)/i.test(tag);
}

function buildKitStyleBlock(name, version, css) {
  return `<style data-kit="${name}" data-kit-version="${version}">\n${css}\n</style>`;
}

// Bake the served kit stylesheet into HTML at publish time. The stored revision
// keeps the inlined bytes so later kit changes never rewrite history.
function inlineKitIntoHtml(html, kit) {
  const source = typeof html === 'string' ? html : String(html);
  const styleBlock = buildKitStyleBlock(kit.name, kit.version, kit.css);
  let replaced = false;
  const withLinksReplaced = source.replace(/<link\b[^>]*>/gi, (tag) => {
    if (!linkHasStylesheetRel(tag) || !linkHasDataKit(tag)) return tag;
    replaced = true;
    return styleBlock;
  });
  if (replaced) return withLinksReplaced;
  // A marked block is kit-managed: replace it with the current served bytes so a
  // revise with `kit` refreshes a stale inline (fresh-eyes review 2026-09-24).
  const markedStyle = /<style\b[^>]*\bdata-kit\s*=[^>]*>[\s\S]*?<\/style>/i;
  if (markedStyle.test(withLinksReplaced)) return withLinksReplaced.replace(markedStyle, () => styleBlock);

  const charsetMatch = withLinksReplaced.match(/<meta\b[^>]*\bcharset\b[^>]*>/i);
  if (charsetMatch && charsetMatch.index != null) {
    const insertAt = charsetMatch.index + charsetMatch[0].length;
    return `${withLinksReplaced.slice(0, insertAt)}\n${styleBlock}${withLinksReplaced.slice(insertAt)}`;
  }
  const headMatch = withLinksReplaced.match(/<head\b[^>]*>/i);
...
function applyKitToFiles(files, kit) {
  if (!kit) {
    return { files, sizeBytes: files.reduce((total, file) => total + file.buffer.length, 0), kit: null };
  }
  const nextFiles = files.map((file) => {
    if (!isHtmlPublishPath(file.path)) return file;
    const html = file.buffer.toString('utf8');
    const inlined = inlineKitIntoHtml(html, kit);
    return { path: file.path, buffer: Buffer.from(inlined, 'utf8') };
  });
  const sizeBytes = nextFiles.reduce((total, file) => total + file.buffer.length, 0);
  return {
    files: nextFiles,
    sizeBytes,
    kit: { name: kit.name, version: kit.version, revision: kit.revision },
  };
}

function createListProjection(repoId, publication) {
  const { revisions, ...projection } = createPublicationProjection(repoId, publication);
  return projection;
}
...
function createPublicationProjection(repoId, publication) {
  ...
  return {
    ...
    entryPath,
    kit: projectKitMeta(current && current.kit),
    ...
    revisions: publication.revisions.map((revision) => ({
      ...
      kit: projectKitMeta(revision.kit),
...
class PublishStore {
  ...
  serializePublicationSummary(publication) {
    return createListProjection(this.repoId, publication);
  }

  // Directory names are the source of truth for which slugs exist; each is
  // re-validated (a stray non-slug directory is simply skipped, not fatal).
  async listSlugs() {
    await this.ensureAreaPath();
    let entries;
    try {
      entries = await fs.readdir(this.areaPath, { withFileTypes: true });
    } catch (error) {
    ...
    for (const entry of entries) {
      ...
      try {
        slugs.push(normalizeSlug(entry.name));
      } catch {
        // Not a slug-shaped directory; ignore.
      }
    }
    return slugs.sort();
  }
  ...
  async listPublications(filter = {}) {
    ...
    for (const slug of slugs) {
      const publication = await this.readPublication(slug);
      if (!publication) continue;
      const state = publication.revokedAt ? 'revoked' : 'active';
  ...
  // A stable revision for the whole listing: changes exactly when any
  // publication's stored state changes (create, update, revoke).
  computeListRevision(publications) {
    const material = publications
      .map((p) => `${p.slug}:${p.updatedAt}:${p.currentRevision}:${p.revokedAt || ''}`)
      .sort()
      .join('|');
  ...
  buildInput(input) {
    ...
    const withKit = applyKitToFiles(normalized.files, input && input.kit);
    if (withKit.sizeBytes > this.limits.maxRevisionBytes) {
      throw codedError(`revision exceeds the ${this.limits.maxRevisionBytes}-byte limit.`, 'ELIMIT');
    }
    return {
      files: withKit.files,
      sizeBytes: withKit.sizeBytes,
      kit: withKit.kit,
      entryPath: normalizeEntryPath(input && input.entryPath, withKit.files),
