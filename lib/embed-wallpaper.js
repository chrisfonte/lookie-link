...
function wallpaperRuntime() {
  ...
  window.addEventListener('message', function (event) {
    ...
    menu.querySelector('[data-wallpaper-previous]').disabled = !state.choices.length;
    menu.querySelector('[data-wallpaper-next]').disabled = !state.choices.length;
    choice.options[0].textContent = state.choices.length ? 'No background' : 'No backgrounds for this theme';
