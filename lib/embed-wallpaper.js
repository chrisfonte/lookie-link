...
function wallpaperRuntime() {
  ...
  var state = null;
  var catalogKey = '';
  // Link-scoped overrides apply once, to the first state the document reports.
  var urlApi = window.lookieLinkUrl || {get: function () { return null; }, set: function () {}};
  var pendingUrl = {choice: urlApi.get('lookie-wallpaper'), opacity: urlApi.get('lookie-panel'), blur: urlApi.get('lookie-blur')};
  ...
  window.addEventListener('message', function (event) {
    ...
    if (pendingUrl.choice || pendingUrl.opacity || pendingUrl.blur) {
      var wanted = pendingUrl;
      pendingUrl = {};
      var ids = state.choices.map(function (item) { return item.id; });
      post({type:'lookie-link:set-wallpaper',
        choice: wanted.choice === 'none' || ids.indexOf(wanted.choice) !== -1 ? wanted.choice : state.choice,
        opacity: wanted.opacity ? Number(wanted.opacity) : state.opacity,
        blur: wanted.blur ? Number(wanted.blur) : state.blur});
    }
  });
  function send() {
    if (!state) return;
    urlApi.set('lookie-wallpaper', choice.value);
    urlApi.set('lookie-panel', Number(opacity.value));
    urlApi.set('lookie-blur', Number(blur.value));
