...
const { baseHtml, themeScript, toolbarHtml, getThemeList, findThemeBySlugOrAlias, isKnownThemeSlug, propertiesMenu } = require('../renderer');
...
function formHeaders(res, cspNonce) {
  ...
  res.set('Content-Security-Policy', [
    ...
    // Viewer-wide wallpapers are same-origin images (/wallpaper/...).
    "img-src 'self'",
...
function formProperties(record, extras = {}) {
  ...
  // A stored value that is an alias (#389) is still installed -- it renders --
  // so it is still reported, but by the theme's canonical LABEL rather than
  // the alias slug (which names nothing an author chose from the picker). A
  // value that already IS the canonical slug is reported as before -- the
  // stored template value itself is never rewritten either way.
  const presentationTheme = presentation.theme && findThemeBySlugOrAlias(presentation.theme);
  if (presentationTheme) {
    rows.push(['Theme', escapeHtml(
      presentationTheme.slug === presentation.theme ? presentation.theme : presentationTheme.label
    )]);
  }
...
function createFormsRouter(options) {
  ...
  // Read the theme CSS on every render: the server passes a live getter that
  // is empty at construction and filled (and refreshed) after createApp.
  // Copying it once shipped every Trackers page without theme tokens
  // (transparent glass over the wallpaper, 2026-09-24).
  const themeCss = () => options.customThemeCss || '';
  ...
  function refuseQueryToken(req, res, next) {
    if (queryCarriesToken(req)) {
      ...
      apiError(res, 400, 'query_credentials_rejected', 'URL credentials are not accepted for forms.');
      return;
    }
    next();
  }
  ...
  function browserEnvelope(req, res, token, type) {
    ...
    if (outcome) {
      ...
      // Browsers keep text/plain; a caller that prefers JSON gets the envelope
      // the OpenAPI document declares (same negotiation as server.js fallbacks).
      if (req.accepts(['text/plain', 'text/html', 'application/json']) === 'application/json') apiError(res, 403, null, 'Request refused.');
      else res.status(403).type('text/plain').send('Request refused.');
      return false;
    }
    return true;
  }
  ...
  function apiAuthenticated(req, res, type = 'form.submit') {
    if (principalFor(req)) return true;
    emitAudit(req, type, 'rejected_authn');
    apiError(res, 401, null, 'Authentication required.');
    return false;
  }
  ...
  function managementBody(req, res, allowedKeys, type) {
    if (!req.is('application/json')) {
      ...
      apiError(res, 415, null, 'Content-Type must be application/json.');
      return null;
    }
  ...
  function managementEnvelope(req, res, mutation, type) {
    ...
    if (bearer && cookieValue(req, CONTEXT_COOKIE)) {
      emitAudit(req, type, 'rejected_authn');
      apiError(res, 400, null, 'Ambiguous credentials are not accepted.');
      return false;
    }
    // Name the missing piece (independent-agent trial 2026-09-23: a bare
    // "Request refused." cost four attempts). The audit already distinguished
    // the two; the caller now gets the same distinction.
    if (mutation && !bearer && !originAllowed(req, publicOrigins)) {
      emitAudit(req, type, 'rejected_origin');
      apiError(res, 403, 'origin_rejected', 'Browser mutations need an Origin header matching a configured public origin; agents present a bearer token instead.');
      return false;
    }
    if (mutation && !bearer && !validCsrf(req, req.get('x-csrf-token'))) {
      emitAudit(req, type, 'rejected_csrf');
      apiError(res, 403, 'csrf_rejected', 'Browser mutations need the form page\'s synchronizer token as X-CSRF-Token with its context cookie; agents present a bearer token instead.');
      return false;
    }
  ...
  router.use('/api/forms', (req, res, next) => {
    apiParser(req, res, (error) => {
      ...
      apiError(res, oversized ? 413 : 400, oversized ? 'payload_too_large' : 'invalid_json',
        oversized ? 'Submission body is too large.' : 'Invalid JSON body.');
    });
  });
  ...
  router.post('/api/forms/templates', async (req, res, next) => {
    ...
    try {
      ...
      if (!resource || !await allowed(req, 'forms.manage', resource)) {
        emitAudit(req, type, 'rejected_authz');
        apiError(res, 403, null, 'Forbidden.');
        return;
      }
  ...
  function validContainerBuilderBody(body) {
    ...
    if (typeof theme === 'string' && theme !== '' && !isKnownThemeSlug(theme)) return false;
  ...
  function validBuilderBody(body, create = false) {
    ...
    if (!create && body && typeof body === 'object') {
      ...
      if (typeof theme === 'string' && theme !== ''
        && !isKnownThemeSlug(theme)) return false;
      const mode = body.themeMode;
      if (typeof mode === 'string' && mode !== '' && mode !== 'dark' && mode !== 'light') return false;
    }
  ...
  async function sendConfigure(res, record, csrfToken, responseOptions = {}, status = 200) {
    ...
    if (record.draft.parentId && !viewingVersion) {
      ...
      if (parent) {
        ...
        const installed = findThemeBySlugOrAlias(effectiveScheme);
  ...
  router.get('/forms', async (req, res, next) => {
    ...
    try {
      ...
      res.status(200).type('html').send(renderFormsIndexPage(
        ...
        {customThemeCss: themeCss(), cspNonce, timezone: serverTimezone, now: currentDate()},
      ));
    } catch (error) {
      next(error);
    }
  ...
  router.get('/forms/entries', async (req, res, next) => {
    ...
    try {
      ...
      res.status(200).type('html').send(renderRootHistoryPage(tagged, issueContext(req, res), {
        customThemeCss: themeCss(), cspNonce, timezone: serverTimezone,
  ...
  router.get('/forms/:templateId', async (req, res, next) => {
    try {
      ...
      if (template.archived === true) {
        ...
        res.status(200).type('html').send(renderArchivedFormPage(template, {customThemeCss: themeCss(), cspNonce, canManage}));
        return;
      }
      if (template.kind === 'container') {
        ...
        res.status(200).type('html').send(renderContainerPage(template, members, {
          customThemeCss: themeCss(), cspNonce, canManage, csrfToken,
      ...
      if (template.kind === 'parent' || activeChildIds.length) {
        ...
        res.status(200).type('html').send(renderParentPage(template, byTitle(children), {
          customThemeCss: themeCss(), cspNonce, canManage, csrfToken, relatedForms,
          timezone: serverTimezone, now: currentDate(),
        }));
        return;
      }
    ...
    try {
      ...
      if (template.kind === 'container') {
        ...
        res.status(200).type('html').send(renderEntriesPage(template, submissions, {
          customThemeCss: themeCss(), cspNonce, timezone: serverTimezone, canManage, csrfToken,
        }));
        return;
      }
  ...
  router.post('/api/forms/:templateId/submissions', async (req, res, next) => {
    try {
      ...
      if (!bearer && !originAllowed(req, publicOrigins)) {
        ...
        apiError(res, 403, 'origin_rejected', 'Browser submissions need an Origin header matching a configured public origin; agents present a bearer token instead.');
        return;
      }
      if (!bearer && !validCsrf(req, req.get('x-csrf-token'))) {
        emitAudit(req, 'form.submit', 'rejected_csrf');
        apiError(res, 403, 'csrf_rejected', 'Browser submissions need the form page\'s synchronizer token as X-CSRF-Token with its context cookie; agents present a bearer token instead.');
        return;
      }
      ...
      if (Object.keys(body).some((key) => !allowedKeys.has(key))) {
        emitAudit(req, 'form.submit', 'rejected_validation', template);
        apiError(res, 400, null, 'Unknown or server-derived submission field.');
        return;
      }
  ...
  router.get('/api/forms/:templateId/submissions', async (req, res, next) => {
    try {
      ...
      if (!Number.isSafeInteger(parsedLimit) || parsedLimit < 1) {
        ...
        apiError(res, 400, null, 'limit must be a positive integer.');
        return;
      }
