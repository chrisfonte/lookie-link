...
async function searchWithRipgrep(options) {
  ...
  if (!repos.length) {
    return { results: [], count: 0, truncated: false, reposSearched: 0, backend: 'ripgrep', limits: { results: limit, fileBytes: MAX_FILE_BYTES } };
  }
  ...
  // Body matches: one match record per file is enough for a result + snippet.
  const cacheKey = JSON.stringify([query.toLowerCase(), roots]);
  const contentRun = await cached(contentCache, cacheKey, CONTENT_CACHE_MS, () => runRipgrep(options.binary, ['--json', '--max-count', '1', ...baseArgs, '--', query, ...roots], options.timeoutMs || RG_TIMEOUT_MS));
  const lowerQuery = query.toLowerCase();
  const found = new Map(); // key repo/path -> result
  for (const line of contentRun.stdout.split('\n')) {
    ...
    if (!mapped || !options.canView(mapped.repo, mapped.relativePath, 'file')) continue;
    const text = record.data.lines && record.data.lines.text ? record.data.lines.text : '';
    const key = `${mapped.repo}/${mapped.relativePath}`;
    found.set(key, { ...mapped, bodyMatches: true, line: record.data.line_number || null, lineText: text });
  }
  // Path matches: a cheap file listing under the same globs.
  const filesRun = await cached(filesCache, JSON.stringify(roots), FILES_CACHE_MS, () => runRipgrep(options.binary, ['--files', ...baseArgs.filter((a) => a !== '-i' && a !== '-F'), '--', ...roots], options.timeoutMs || RG_TIMEOUT_MS));
  for (const absolute of filesRun.stdout.split('\n')) {
    if (!absolute) continue;
    const mapped = mapPath(absolute);
    if (!mapped || !mapped.relativePath.toLowerCase().includes(lowerQuery)) continue;
    if (!options.canView(mapped.repo, mapped.relativePath, 'file')) continue;
    const key = `${mapped.repo}/${mapped.relativePath}`;
    const existing = found.get(key);
    if (existing) existing.pathMatches = true;
    else found.set(key, { ...mapped, pathMatches: true, bodyMatches: false, line: null, lineText: '' });
  }
  ...
  for (const item of found.values()) {
    ...
    const source = item.lineText || '';
    const index = source.toLowerCase().indexOf(lowerQuery);
    const snip = index < 0
      ? source.slice(0, 160).replace(/\s+/g, ' ').trim()
      : source.slice(Math.max(0, index - 60), Math.min(source.length, index + query.length + 100)).replace(/\s+/g, ' ').trim();
    results.push({
      repo: item.repo,
  ...
  const truncated = results.length > limit || contentRun.killed || filesRun.killed;
  return {
