...
// Each parsed entry can hold tens of thousands of match objects for a common
// term across the fleet: 20 distinct unscoped searches grew the process from
// 380 MB to 1.0 GB with the old 200-entry cap (2026-09-24). Keep the cache
// small and evict oldest-first on every insert.
...
async function searchWithRipgrep(options) {
  ...
  const lowerQueryForWindow = primary.lower;
  const content = await cached(contentCache, JSON.stringify(['match', primary.lower, roots]), CONTENT_CACHE_MS, async () => {
    ...
    for (const line of run.stdout.split('\n')) {
      ...
      if (!mapped) continue;
      // Keep only a window around the first match: a single-line JSON or
      // minified file can be hundreds of KB per line, and a common term across
      // the fleet held ~370 MB of such lines in large_object_space (2026-09-24).
      const fullLine = record.data.lines && record.data.lines.text ? record.data.lines.text : '';
      const sub = record.data.submatches && record.data.submatches[0];
      const at = sub && Number.isFinite(sub.start) ? sub.start : fullLine.toLowerCase().indexOf(lowerQueryForWindow);
      const lineText = fullLine.length <= 400 ? fullLine : fullLine.slice(Math.max(0, at - 120), Math.max(0, at - 120) + 400);
      matches.push({ ...mapped, line: record.data.line_number || null, lineText });
    }
...
module.exports = { resolveRipgrep, searchWithRipgrep, SEARCHABLE_GLOBS, MAX_RESULT_LIMIT, contentCacheSize: () => contentCache.size };
