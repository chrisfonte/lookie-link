...
function runRipgrep(binary, args, timeoutMs) {
...
async function searchWithRipgrep(options) {
  ...
  const contentRun = await runRipgrep(options.binary, ['--json', '--max-count', '1', ...baseArgs, '--', query, ...roots], options.timeoutMs || RG_TIMEOUT_MS);
  ...
  const filesRun = await runRipgrep(options.binary, ['--files', ...baseArgs.filter((a) => a !== '-i' && a !== '-F'), '--', ...roots], options.timeoutMs || RG_TIMEOUT_MS);
