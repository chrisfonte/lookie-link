...
async function searchWithRipgrep(options) {
  ...
  // Parsed (not raw) results are cached: the content matches for this query,
  // and the mapped file list for these roots. Caller filtering runs per
  // request on top of the cached, unfiltered arrays.
  const content = await cached(contentCache, JSON.stringify([lowerQuery, roots]), CONTENT_CACHE_MS, async () => {
    const run = await runRipgrep(options.binary, ['--json', '--max-count', '1', ...baseArgs, '--', query, ...roots], options.timeoutMs || RG_TIMEOUT_MS);
    const matches = [];
  ...
  const files = await cached(filesCache, rootsKey, FILES_CACHE_MS, async () => {
    const run = await runRipgrep(options.binary, ['--files', ...baseArgs.filter((a) => a !== '-i' && a !== '-F'), '--', ...roots], options.timeoutMs || RG_TIMEOUT_MS);
    const entries = [];
  ...
  for (const m of content.matches) {
    ...
    found.set(`${m.repo}/${m.relativePath}`, { repo: m.repo, relativePath: m.relativePath, bodyMatches: true, line: m.line, lineText: m.lineText });
  }
  for (const e of files.entries) {
    if (!e.lower.includes(lowerQuery)) continue;
  ...
  const contentRun = { killed: content.killed };
  const filesRun = { killed: files.killed };
