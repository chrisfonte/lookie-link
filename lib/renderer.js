...
const { wallpaperControlsHtml, wallpaperControlsScript } = require('./embed-wallpaper');
const { hasWallpapers, viewerWallpaperHtml, viewerWallpaperScript } = require('./viewer-wallpaper');
...
function resolvePortableViewHref(href, context = {}) {
  ...
  const sourceDir = path.join(repoRoot, repoRootRelative(context, currentDir));
  let absoluteTarget;
  ...
  let normalizedRelativePath = relativePath === '.' ? '' : relativePath;
  // A virtual root (published slug): the disk path lacks the prefix, the URL needs it.
  if (context.repoRootPrefix && targetRepo.repo === context.repo && path.resolve(targetRepo.rootPath) === repoRoot) {
    normalizedRelativePath = normalizedRelativePath ? `${context.repoRootPrefix}/${normalizedRelativePath}` : context.repoRootPrefix;
  }
...
// A virtual repo (the published store) serves <slug>/<file> from a root that
// holds only <file>. context.repoRootPrefix names that leading URL segment so
// disk checks strip it while hrefs keep it. Without this the fallback loop in
// resolveLocalAssetHref dropped the slug from every published <img> (2026-09-23).
function repoRootRelative(context, urlRelativePath) {
  const prefix = context.repoRootPrefix;
  if (!prefix) return urlRelativePath;
  if (urlRelativePath === prefix) return '';
  return urlRelativePath.startsWith(prefix + '/') ? urlRelativePath.slice(prefix.length + 1) : urlRelativePath;
}
...
function resolveLocalAssetHref(src, context, allowedExtensions) {
  ...
  // Already a served URL (an earlier pass, e.g. rewritePortableNavigationLinks,
  // produced it). Prefixing it again yielded /view/<repo>/view/<repo>/... on
  // every markdown link to an image/audio/video file (found 2026-09-23).
  if (/^\/(view|asset|raw)\//.test(src)) {
    return null;
  }
  ...
  if (src.startsWith('~/')) {
    ...
    if (context.repoRoot) {
      const candidate = path.join(context.repoRoot, repoRootRelative(context, joined));
      if (!fs.existsSync(candidate)) {
        ...
        if (tailMatch) {
          ...
          for (let attempts = 0; attempts < 10 && dir; attempts++) {
            ...
            const tryFull = path.join(context.repoRoot, repoRootRelative(context, tryPath));
...
function renderPreviewHtml({
  ...
  const rendered = renderContent(relativePath, source, {
    ...
    repoRootPrefix: repoRootPrefix || null,
...
// Looks a slug up by its canonical value OR any alias (#389) and returns the
// matching theme entry, or null. Every membership check against the theme
// list must go through this (or isKnownThemeSlug) so an alias is treated as
// installed everywhere a canonical slug is.
function findThemeBySlugOrAlias(slug) {
  if (typeof slug !== 'string' || slug === '') return null;
  for (const theme of getThemeList()) {
    if (theme.slug === slug || (theme.aliases || []).includes(slug)) return theme;
  }
  return null;
}

function isKnownThemeSlug(slug) {
  return findThemeBySlugOrAlias(slug) !== null;
}
...
function toolbarHtml(extraButtons = '', options = {}) {
  ...
  // An alias (#389) never gets its own picker button, but it does get its own
  // label span -- a page whose data-color-scheme is set to the alias (a saved
  // choice before the client remaps it, or a template's presentation.theme)
  // still reveals the theme's real name instead of nothing.
  const themeNames = themes.map((theme) => {
    const labelHtml = escapeHtml(theme.label);
    const slugs = [theme.slug, ...(theme.aliases || [])];
    return slugs.map((slug) => `<span data-theme-name="${escapeHtml(slug)}">${labelHtml}</span>`).join('');
  }).join('');
  ...
  return `<div class="viewer-toolbar" role="toolbar" aria-label="Viewer controls">
    ...
    ${options.wallpaperControls === true || (options.wallpaperControls !== false && hasWallpapers()) ? wallpaperControlsHtml() : ''}
...
// The head bootstrap and toolbar use the same URL validation before and after paint.
function themeSelectionScript() {
  const themeAliasMap = {};
  for (const theme of getThemeList()) {
    for (const alias of theme.aliases || []) {
      themeAliasMap[alias] = theme.slug;
    }
  }
  return `
      var themeAliasMap = ${JSON.stringify(themeAliasMap)};
      var themeSlugs = ${JSON.stringify(getThemeList().map((theme) => theme.slug))};
      ...
      function validScheme(value) {
        var slug = canonicalScheme(value);
        return themeSlugs.indexOf(slug) !== -1 ? slug : null;
      }
      function queryValue(params, key, validate) {
        var values = params.getAll(key);
        return values.length === 1 ? validate(values[0]) : null;
      }
      function validMode(value) {
        return value === 'light' || value === 'dark' ? value : null;
      }
      // Wallpaper selection rides on the same link-scoped contract as the theme:
      // lookie-wallpaper=<id|none>, lookie-panel=<50-100>, lookie-blur=<0-16>.
      function validWallpaper(value) {
        return value === 'none' || /^[a-z0-9][a-z0-9-]{0,63}$/.test(value) ? value : null;
      }
      function validRange(min, max) {
        return function (value) {
          var n = /^\\d{1,3}$/.test(value) ? Number(value) : NaN;
          return n >= min && n <= max ? String(n) : null;
        };
      }
      var query = new URL(window.location.href).searchParams;
      var urlScheme = queryValue(query, 'lookie-scheme', validScheme);
      var urlMode = queryValue(query, 'lookie-theme', validMode);
      // Every link-scoped selection, keyed by its query parameter. A null value
      // means the URL did not carry it; links then inherit nothing for that key.
      var urlSelection = {
        'lookie-scheme': urlScheme,
        'lookie-theme': urlMode,
        'lookie-wallpaper': queryValue(query, 'lookie-wallpaper', validWallpaper),
        'lookie-panel': queryValue(query, 'lookie-panel', validRange(50, 100)),
        'lookie-blur': queryValue(query, 'lookie-blur', validRange(0, 16))
      };
      var urlValidators = {
        'lookie-scheme': validScheme,
        'lookie-theme': validMode,
        'lookie-wallpaper': validWallpaper,
        'lookie-panel': validRange(50, 100),
        'lookie-blur': validRange(0, 16)
      };

      function canonicalScheme(slug) {
        return Object.prototype.hasOwnProperty.call(themeAliasMap, slug) ? themeAliasMap[slug] : slug;
      }
`;
}
...
function themeScript(cspNonce = null, defaults = {}) {
  ...
  <script${nonceAttribute(cspNonce)}>
    (function () {
      ...
      var storedScheme = readPreference('lookie-link-color-scheme') || 'slate';
      var canonicalStoredScheme = canonicalScheme(storedScheme);
      if (canonicalStoredScheme !== storedScheme) {
        try { localStorage.setItem('lookie-link-color-scheme', canonicalStoredScheme); } catch (e) {}
      }
      var savedScheme = urlScheme || pageScheme || canonicalStoredScheme;
      var savedTheme = urlMode || pageMode || validMode(readPreference('lookie-link-theme')) || 'dark';
      applyColorScheme(savedScheme);
      applyTheme(savedTheme);

      // Decorate real hrefs so copied links, middle clicks and new tabs agree.
      // Keep each authored URL separate from our last rewrite so picker changes
      // can update inherited values without overwriting explicit destinations.
      var themedLinks = new WeakMap();
      function carryTheme(link) {
        var href = link.getAttribute('href');
        var previous = themedLinks.get(link);
        var original = previous && previous.last === href ? previous.original : href;
        if (!original || original.charAt(0) === '#' || link.hasAttribute('download')) return;
        var target;
        try { target = new URL(original, document.baseURI); } catch (_) { return; }
        if (target.origin !== window.location.origin ||
            !(target.pathname === '/' || /^\\/(view|edit|forms)(\\/|$)/.test(target.pathname))) return;
        var carried = false;
        Object.keys(urlSelection).forEach(function (key) {
          if (!urlSelection[key]) return;
          carried = true;
          if (!queryValue(target.searchParams, key, urlValidators[key])) target.searchParams.set(key, urlSelection[key]);
        });
        if (!carried && !previous) return;
        var next = target.href;
        themedLinks.set(link, { original: original, last: next });
        if (href !== next) link.setAttribute('href', next);
      }
      function carryThemeLinks() {
        document.querySelectorAll('a[href]').forEach(carryTheme);
      }
      carryThemeLinks();
      new MutationObserver(function (records) {
        records.forEach(function (record) {
          if (record.type === 'attributes') carryTheme(record.target);
          else record.addedNodes.forEach(function (node) {
            if (node.nodeType !== 1) return;
            if (node.matches('a[href]')) carryTheme(node);
            node.querySelectorAll('a[href]').forEach(carryTheme);
          });
        });
      }).observe(document.body, { childList: true, subtree: true, attributes: true, attributeFilter: ['href'] });
      // A control change rewrites the address only for a key the URL already
      // carried, so a plain URL stays plain and a themed link stays honest.
      function updateUrlSelection(key, value) {
        if (!Object.prototype.hasOwnProperty.call(urlSelection, key) || !urlSelection[key]) return;
        urlSelection[key] = String(value);
        if (key === 'lookie-scheme') urlScheme = value;
        if (key === 'lookie-theme') urlMode = value;
        var current = new URL(window.location.href);
        current.searchParams.set(key, String(value));
        window.history.replaceState(window.history.state, '', current.href);
        carryThemeLinks();
      }
      // The wallpaper runtimes (viewer-wide and embed bridge) read and update
      // their link-scoped values through this one door.
      window.lookieLinkUrl = {
        get: function (key) { return urlSelection[key] || null; },
        set: updateUrlSelection
      };
      ...
      if (themeMenuEl) {
        ...
        // The attribute may be an alias (a page-level theme, #389) -- it is left
        // as-is on the root, but the picker's highlight must land on the
        // canonical item, since the menu only lists canonical slugs.
        labelTheme(canonicalScheme(rootEl.getAttribute('data-color-scheme') || 'slate'));
        ...
        themeMenuEl.addEventListener('click', function (event) {
          ...
          savePreference('lookie-link-color-scheme', item.value);
          updateUrlSelection('lookie-scheme', item.value);
      ...
      if (themeToggleBtn) {
        themeToggleBtn.addEventListener('click', function () {
          ...
          applyTheme(next);
          savePreference('lookie-link-theme', next);
          updateUrlSelection('lookie-theme', next);
        });
      }
...
// wallpaper: 'viewer' (default) paints the viewer-wide picture behind the page;
// 'embed' leaves it to the embedded document's own bridge; false disables.
function baseHtml({ title, body, customThemeCss, cspNonce = null, defaultScheme = null, defaultMode = null, wallpaper = 'viewer' }) {
...
<head>
  ...
  <script${nonceAttribute(cspNonce)}>
    (function () {
      ...
      root.setAttribute('data-color-scheme', urlScheme || pageScheme || canonicalScheme(readPreference('lookie-link-color-scheme')) || 'slate');
      if ((urlMode || pageMode || readPreference('lookie-link-theme')) === 'light') root.setAttribute('data-theme', 'light');
    }());
  </script>
</head>
<body>
  ${wallpaper === 'viewer' ? viewerWallpaperHtml() : ''}
  ${body}
  ${wallpaper === 'viewer' ? viewerWallpaperScript(cspNonce) : ''}
...
// Rendered-body cache (API roadmap R9, 2026-09-23). Markdown → sanitize → link
// rewriting is the expensive part of a document page (hundreds of ms for a
// long session document) and is pure in (source, path, repo, link context).
// The server passes `renderCacheKey` only for callers whose link resolution
// is not scoped (unrestricted), so a scoped caller never sees another's
// rewrite. Bounded LRU by entry count and total bytes.
...
function renderCacheGet(key) {
  ...
  if (!hit) return null;
  renderCache.delete(key); renderCache.set(key, hit); // refresh recency
  return hit;
}
function renderCacheSet(key, value) {
  const bytes = Buffer.byteLength(value.html, 'utf8');
  if (bytes > RENDER_CACHE_MAX_BYTES / 4) return; // never let one page dominate
  renderCache.set(key, { ...value, bytes });
  renderCacheBytes += bytes;
  while ((renderCache.size > RENDER_CACHE_MAX_ENTRIES || renderCacheBytes > RENDER_CACHE_MAX_BYTES) && renderCache.size) {
    const [oldest, entry] = renderCache.entries().next().value;
    renderCache.delete(oldest); renderCacheBytes -= entry.bytes;
  }
}
function renderCacheStats() { return { entries: renderCache.size, bytes: renderCacheBytes }; }
function renderCacheClear() { renderCache.clear(); renderCacheBytes = 0; }

function renderDocumentPage({ repo, repoRoot, repoRootPrefix = null, repoMappings = null, canResolveLink = null, relativePath, source, parentHref, mtime, size, editHref = null, rawHtmlHref = null, embedHtmlHref = null, customThemeCss, queryToken = null, annotationsEnabled = false, homePath = null, renderCacheKey = null }) {
  ...
  const cacheKey = renderCacheKey
    ? `${renderCacheKey}|${repo}|${relativePath}|${queryToken || ''}|${annotationsEnabled ? 1 : 0}|${homePath || ''}|${require('node:crypto').createHash('sha256').update(source).digest('hex')}`
    : null;
  const cachedBody = cacheKey ? renderCacheGet(cacheKey) : null;
  let rendered;
  if (cachedBody) {
    rendered = { kind: cachedBody.kind, html: cachedBody.html };
  } else {
    rendered = renderContent(relativePath, source, {
      repo,
      repoRoot: repoRoot || null,
      repoRootPrefix: repoRootPrefix || null,
      repoMappings: repoMappings || null,
    ...
    if (cacheKey) renderCacheSet(cacheKey, { kind: rendered.kind, html: rendered.html });
  }
  ...
  if (usesEmbeddedHtml) {
    ...
    /* Solid ground, no bordered box: the frame owns the whole viewport, so the
       page gradient would only ever show in the header strip and the 1px
       .content border drew a seam between chrome and document (wallpapers made
       it obvious). The frame's own background is already var(--bg). */
    body:has(main.layout[data-render-mode="viewport"]) { overflow: hidden; margin: 0; background: var(--bg); }
    main.layout[data-render-mode="viewport"] > .html-embed-view { border: 0; border-radius: 0; }
    ...
    const body = `${toolbarHtml(extraButtons, {wallpaperControls: true})}${viewportModeStyle}
