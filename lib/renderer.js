...
// lightbox); absence of the hint keeps today's content-height seamless reader. The
// check is deliberately tolerant so it agrees pre-parse (a regex over raw source in
// renderDocumentPage) and post-parse (JSDOM in transformEmbedHtml sees the same
// attribute). Accepts <html data-lookie-render="viewport"> and
// <meta name="lookie-render" content="viewport"> (either attribute order).
function detectRenderMode(source) {
  const text = String(source || '');
  const htmlAttr = /<html\b[^>]*\bdata-lookie-render\s*=\s*["']?\s*viewport\b/i;
  const metaNameFirst = /<meta\b[^>]*\bname\s*=\s*["']lookie-render["'][^>]*\bcontent\s*=\s*["']?\s*viewport\b/i;
  const metaContentFirst = /<meta\b[^>]*\bcontent\s*=\s*["']?\s*viewport\b[^>]*\bname\s*=\s*["']lookie-render["']/i;
  return htmlAttr.test(text) || metaNameFirst.test(text) || metaContentFirst.test(text)
    ? 'viewport'
    : 'default';
}
