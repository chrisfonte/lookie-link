...
function themeScript(cspNonce = null, defaults = {}) {
  ...
  // Alias -> canonical slug (#389), so a saved picker choice made before a
  // rename still resolves and re-selects the canonical entry. A page-level
  // theme (a form template's presentation.theme) is left as-is -- it already
  // renders via the alias's own CSS block, and remapping it here would leak
  // the rewrite into the stored global preference.
  const themeAliasMap = {};
  for (const theme of getThemeList()) {
  ...
  <script${nonceAttribute(cspNonce)}>
    (function () {
      ...
      var storedScheme = localStorage.getItem('lookie-link-color-scheme') || 'slate';
      ...
      var savedScheme = pageScheme || canonicalStoredScheme;
