...
// Extract YouTube iframes from source markdown BEFORE markdown-it renders
// (linkify: true converts URLs inside escaped attrs into <a> tags, breaking
// post-processing).  Returns { cleaned, iframes } where cleaned has
// placeholders and iframes maps placeholder → safe HTML.
function extractYouTubeEmbeds(source) {
  const iframes = {};
  let counter = 0;
  const cleaned = source.replace(
    /<iframe\b[\s\S]*?src="(https:\/\/www\.youtube\.com\/embed\/[A-Za-z0-9_-]+)"[\s\S]*?<\/iframe>/gi,
    (match, src) => {
      const key = `YTEMBED${counter++}YTEMBED`;
      iframes[key] = `<iframe width="560" height="315" src="${escapeHtml(src)}" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen style="max-width:100%; border-radius:8px; margin:1em 0;"></iframe>`;
      return key;
    }
  );
  return { cleaned, iframes };
}

function restoreYouTubeEmbeds(html, iframes) {
  if (!iframes || !Object.keys(iframes).length) return html;
  let output = html;
  for (const [placeholder, iframe] of Object.entries(iframes)) {
    // markdown-it wraps the comment in a <p>, so replace the whole thing
    output = output.replace(`<p>${placeholder}</p>`, iframe);
...
function postProcessHtml(html, options) {
  let output = html;
  output = restoreYouTubeEmbeds(output, options._ytIframes);
...
function renderContent(filePath, source, options = {}) {
  ...
  if (MARKDOWN_EXTENSIONS.has(extension)) {
    const { cleaned, iframes } = extractYouTubeEmbeds(source);
    const opts = { ...options, _ytIframes: iframes };
    return {
      kind: 'markdown',
      html: postProcessHtml(markdown.render(cleaned), opts),
    };
  }
