...
function toYouTubeEmbedSrc(rawUrl) {
  if (!rawUrl) {
    return null;
  }
...
function createYouTubeEmbedElement(document, embedSrc) {
  ...
  iframe.setAttribute('style', 'display:block; width:100%; max-width:100%; aspect-ratio:16 / 9; height:auto; border-radius:8px; margin:1em 0;');
  return iframe;
}

function autoEmbedYouTubeUrls(html) {
  ...
  for (const anchor of anchors) {
    ...
    const embedSrc = toYouTubeEmbedSrc(anchor.getAttribute('href'));
    if (!embedSrc) {
      continue;
    }

    anchor.replaceWith(createYouTubeEmbedElement(document, embedSrc));
  }
  ...
  for (const node of textNodes) {
    ...
    if (!parent || parent.closest('a, pre, code, script, style, iframe')) {
      continue;
    }

    const text = node.nodeValue || '';
    const regex = /(https?:\/\/(?:www\.)?(?:youtube\.com\/watch\?[^\s<]+|youtu\.be\/[^\s<]+))/gi;
    let match;
    let cursor = 0;
    let changed = false;
    ...
    while ((match = regex.exec(text)) !== null) {
      ...
      const candidate = full ? full[1] : raw;
      const trailing = full && full[2] ? full[2] : '';
      const embedSrc = toYouTubeEmbedSrc(candidate);
      if (!embedSrc) {
        continue;
      }
      ...
      fragment.append(createYouTubeEmbedElement(document, embedSrc));
      if (trailing) {
...
function postProcessHtml(html, options) {
  ...
  output = autoEmbedYouTubeUrls(output);
