...
function renderDocumentPage({ repo, repoRoot, relativePath, source, parentHref, mtime, size }) {
  ...
  const body = `<div class="viewer-toolbar" role="toolbar" aria-label="Viewer controls">
    ${hasToc ? '<button type="button" class="toolbar-btn" data-toc-toggle aria-label="Toggle table of contents" aria-expanded="false" aria-controls="toc-panel">☰</button>' : ''}
    ...
    ${hasToc ? `<aside class="toc-panel" id="toc-panel" data-toc-panel aria-hidden="true" style="display:none">
      <p class="toc-panel-title">Contents</p>
      <nav class="toc-list" data-toc-list aria-label="Table of contents"></nav>
    </aside>` : /* hasToc */ ''}
  </div>
  ${hasToc ? '<div class="toc-overlay" data-toc-overlay aria-hidden="true" style="display:none"></div>' : ''}
  <main class="layout">
    <header class="topbar">
  ...
  <script>
    (function () {
      ...
      if (tocToggleBtn && tocPanel && tocOverlay && tocList && tocContainer) {
        ...
        if (tocHeadings.length >= 3) {
          ...
          tocHeadings.forEach(function (heading) {
            ...
            var level = heading.tagName.match(/^H[1-4]$/i) ? Number(heading.tagName.slice(1)) : 2;
            item.type = 'button';
