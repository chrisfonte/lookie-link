...
function renderDocumentPage({ repo, repoRoot, relativePath, source, parentHref, mtime, size, editHref = null }) {
  ...
  <main class="layout">
    <header class="topbar">
      ...
      ${editHref ? `<p><a class="button-link" href="${escapeHtml(editHref)}">Edit</a></p>` : ''}
...
function renderImagePage({ repo, relativePath, parentHref, imageHref, mtime, size }) {
  const heading = `${repo}/${relativePath}`;
  const body = `<main class="layout">
    <header class="topbar">
      <h1>${escapeHtml(path.basename(relativePath))}</h1>
  ...
  const body = `<main class="layout">
    <header class="topbar">
      ...
      <p class="edit-actions">
        <a class="button-link" href="${escapeHtml(viewHref)}">View file</a>
        <button type="button" class="button-link button-link-primary" data-save-btn>Save</button>
      </p>
      <p class="notice" data-save-status hidden></p>
    </header>

    <section class="editor-tabs" role="tablist" aria-label="Editor panels">
      <button type="button" class="button-link button-link-tab is-active" data-tab-btn data-target="edit" role="tab" aria-selected="true">Edit</button>
      <button type="button" class="button-link button-link-tab" data-tab-btn data-target="preview" role="tab" aria-selected="false">Preview</button>
    </section>
