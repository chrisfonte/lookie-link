  ...
  <script${nonceAttribute(cspNonce)}>
    (function () {
      ...
      function carryTheme(link) {
        ...
        if (urlScheme && !queryValue(target.searchParams, 'lookie-scheme', validScheme)) {
          target.searchParams.set('lookie-scheme', urlScheme);
        }
        if (urlMode && !queryValue(target.searchParams, 'lookie-theme', validMode)) {
          target.searchParams.set('lookie-theme', urlMode);
        }
        if (!urlScheme && !urlMode && !previous) return;
      ...
      function updateUrlSelection(key, value) {
        if (key === 'lookie-scheme' && urlScheme) urlScheme = value;
        else if (key === 'lookie-theme' && urlMode) urlMode = value;
        else return;
