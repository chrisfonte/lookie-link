# ops-file-viewer — Web File Viewer for Operations System

## What to Build

A lightweight web file viewer that runs on the local machine (macOS gateway) and serves files over Tailscale for mobile/local access. The primary use case: an AI agent modifies a file, shows the user a clickable link, and the user can view the rendered file in their browser.

## Core Requirements

1. **Web server** that serves files from configurable root directories (e.g., `~/operations`, `~/operations-fontastic`, `~/operations-chris-fonte`, `~/clawd`)
...
6. **URL structure**: `http://<hostname>:<port>/view/<repo-path>/<file-path>`
   - Example: `http://mac-mini-2.bobcat-tetra.ts.net:9876/view/operations/docs/meta/credentials-management/credentials-management-best-practices.md`
   - Example: `http://mac-mini-2.bobcat-tetra.ts.net:9876/view/clawd/MEMORY.md`
...
- **Hostname**: `mac-mini-2.bobcat-tetra.ts.net` (Tailscale MagicDNS)
...
/view/operations-fontastic/...→ ~/operations-fontastic/...  
/view/operations-chris-fonte/...→ ~/operations-chris-fonte/...
/view/clawd/...               → ~/clawd/...
```
