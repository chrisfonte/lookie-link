...
| Search: `/api/search` | `GET` | Effective `view`; results are caller-filtered | Always | Unknown query parameters are rejected (400 with `details[]`); `repo` is an alias of `scope`. Requires `q`; searches bounded path/content candidates in supported text formats. [`server.js`](../server.js) |
