...
For Paperclip-managed cross-company access, configure a private grant store and a separate admin token set for the lifecycle API:
...
- `storePath` may be `.json`, `.yaml`, or `.yml`; writes are atomic.
- `projectionPath` is optional. When set, Lookie-Link writes an atomic read-only projection of active grants containing only enforcement fields plus `tokenHash`.
- `repoOwners` is required so grant issuance can enforce source-company ownership for each repo.
- `adminTokens` are only for `/api/grants*`; grant bearer secrets are generated by the server and returned once on grant creation or renewal.
- Managed grants store only `tokenHash`, never the raw bearer token.
- Cross-company grant requests must also send `adapterAllowRoots` in the `POST /api/grants` body so Lookie-Link can reject scopes outside the Paperclip adapter's filesystem allow roots.
